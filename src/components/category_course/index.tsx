...
const CategoryCourse: React.FC = () => {
  // THCS
  const arrAltImgTHCS: string[] = ["Lớp 6", "Lớp 7", "Lớp 8", "Lớp 9"];
  const arrLinkImgTHCS: string[] = ["https://hocthongminh.com/_next/image/?url=%2Fimages%2FclassNumber%2Flop-6.png&w=1920&q=90",
  ...
  const arrDescribeTHCS: string[] = ["Tổng hợp tài liệu các môn Toán, Văn, Tiếng Anh, Lịch Sử,... đầy đủ và chi tiết nhất theo chương trình học lớp 6 và các đề thi học kỳ 1, học kỳ 2 kèm theo lời giải chi tiết.",
  ...
  const arrIndexTHCS: number[] = [6, 7, 8, 9];
  // THPT
  const arrAltImgTHPT: string[] = ["Lớp 10", "Lớp 11", "Lớp 12"];
  const arrLinkImgTHPT: string[] = ["https://hocthongminh.com/_next/image/?url=%2Fimages%2FclassNumber%2Flop-10.png&w=1920&q=90",
  ...
  const arrDescribeTHPT: string[] = ["Học online các môn Toán, Lý, Hóa, Sinh, Lịch sử, GDCD, Địa Lý, Văn, Tiếng Anh theo chương trình lớp 10 mới nhất dưới hình thức trắc nghiệm. Các em có thể học, luyện theo chương trình học của bộ GDDT và làm các bài kiểm tra học kỳ 1, học kỳ 2 ngay trên website Học Thông Minh",
  ...
  const arrIndexTHPT: number[] = [10, 11, 12];
    ...
    <div className="category-course">
      ...
      <div style={{ margin: "30px" }}>
        ...
        <Grid container spacing={1} className="grid-container-course">
          {
            arrIndexTHCS.map((itemArr: number, indexArr: number) => {
              return (
                <Grid item xs={6} sm={6} md={3} key={indexArr}>
                  <CardCourse
